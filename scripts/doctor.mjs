...
// Surfaces every common setup gotcha *before* uvicorn or next try to
// start, with actionable error messages.
//
...
// .env.example. Keep in sync with services/api/main.py REQUIRED_SETTINGS
...
function checkEnv() {
  ...
  if (placeholders.length > 0) {
    fail(
      ...
      "Edit .env and replace placeholders with your real credentials (B2: https://secure.backblaze.com/app_keys.htm?utm_source=github&utm_medium=referral&utm_campaign=ai_artifacts&utm_content=b2ai-product-photo-studio · OpenAI: https://platform.openai.com/api-keys)",
    );
  }
}
