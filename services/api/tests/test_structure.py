...
def test_genblaze_only_in_repo():
    """Verify genblaze_* (provider orchestration) is only imported in app/repo/.

    Mirrors the boto3 boundary: all Genblaze imports (genblaze_core /
    genblaze_openai / genblaze_s3) live in app/repo/studio_pipeline.py so the
    service and runtime layers never depend on the SDK directly.
    """
    violations = []
    for layer in LAYER_ORDER:
        if layer == "repo":
        ...
        for pyfile in _get_python_files(layer_dir):
            for imp in _get_imports(pyfile):
                if imp == "genblaze_core" or imp.startswith("genblaze_"):
                    rel = pyfile.relative_to(APP_ROOT.parent)
                    violations.append(f"{rel}: genblaze_* imported outside repo/ ({imp})")
