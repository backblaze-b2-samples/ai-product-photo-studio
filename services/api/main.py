...
from app.runtime import files, health, metrics, skus, upload  # noqa: E402

# --- Startup validation ---
# Required settings are declared with empty-string defaults so that
...
# check above but every call will still fail. Catch that here.
...
app = FastAPI(
    title="AI Product Photo Studio API",
    description=(
        "Generate studio-quality product photos per SKU with gpt-image-1, "
        "stored on Backblaze B2 with SHA-256 provenance."
    ),
    version="0.1.0",
    lifespan=lifespan,
)
...
app.include_router(skus.router, tags=["studio"])
