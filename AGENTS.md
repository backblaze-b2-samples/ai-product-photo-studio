...
AI Product Photo Studio generates studio-quality product photos per SKU. A
seller uploads one reference photo, writes a scene prompt, and the app produces
N reference-faithful variants (angles / seasonal looks) via OpenAI gpt-image-1,
orchestrated through the **Genblaze SDK** and written to Backblaze B2 with a
SHA-256 provenance manifest per run. The core flow:

```
runtime/skus.py  →  service/studio.py  →  repo/studio_pipeline.py
  POST /skus/{sku}/generate                Pipeline → DalleProvider (gpt-image-1
                                           edits, external_inputs=reference)
                                           → ObjectStorageSink (B2, HIERARCHICAL)
```

Object layout in B2:

```
uploads/<sku>/reference/<filename>               # uploaded product photo
skus/<sku>/generations/<run_id>/<variant>.png    # generated shots (Genblaze)
...
apps/web/          Next.js 16 frontend (App Router, Tailwind v4, shadcn/ui)
                   /studio  /library  /files  /upload  / (dashboard)  /settings
services/api/      FastAPI backend (layered: types/config/repo/service/runtime)
                   repo/studio_pipeline.py = the Genblaze image pipeline
...
## 2. Surface contract (what to keep vs adapt)
...
- **File Explorer.** `/files` route, `apps/web/src/app/files/`, and `apps/web/src/components/files/` — full-bucket browse/preview/download/delete. The Files sidebar entry stays.
- **Upload.** `/upload` route + `apps/web/src/components/upload/` — also reused for reference-photo upload (a `sku` form field targets `uploads/<sku>/reference/`).
- **Studio + Library.** `/studio` (`components/studio/`) is the generation surface; `/library` (`components/library/`) is the per-SKU scoped explorer. Both are core to this app — do not remove.

**App-specific (this is where the product logic lives)**
- **Dashboard.** `/` route and `apps/web/src/components/dashboard/` are themed to the SKU / storage-curve story (SKU count, generated shots, total storage, cumulative storage-growth chart, recent-generations table). New aggregations flow through `runtime → service → repo` and TanStack Query hooks in `apps/web/src/lib/queries.ts` — no bare `useEffect + fetch`.
- Update the matching doc in `docs/features/` in the same PR as any change (see §9).
...
- **No `genblaze_*` outside `repo/`** — all provider-orchestration imports
  (`genblaze_core` / `genblaze_openai` / `genblaze_s3`) live only in
  `app/repo/studio_pipeline.py`, mirroring the boto3 rule. The service and
  runtime layers consume plain dicts / typed models, never Genblaze types.
